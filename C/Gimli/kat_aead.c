...
int generate_test_vectors()
{
	...
	unsigned char       key[crypto_aead_KEYBYTES];
	unsigned char		nonce[crypto_aead_NPUBBYTES];
	unsigned char       msg[MAX_MESSAGE_LENGTH];
	unsigned char       msg2[MAX_MESSAGE_LENGTH + crypto_aead_ABYTES];
	unsigned char		ad[MAX_ASSOCIATED_DATA_LENGTH];
	unsigned char		ct[MAX_MESSAGE_LENGTH + crypto_aead_ABYTES];
	...
	sprintf(fileName, "LWC_AEAD_KAT_%d_%d.txt", (crypto_aead_KEYBYTES * 8), (crypto_aead_NPUBBYTES * 8));
	...
	for (unsigned long long mlen = 0; (mlen <= MAX_MESSAGE_LENGTH) && (ret_val == KAT_SUCCESS); mlen++) {
		...
		for (unsigned long long adlen = 0; adlen <= MAX_ASSOCIATED_DATA_LENGTH; adlen++) {
			...
			fprint_bstr(fp, "Key = ", key, crypto_aead_KEYBYTES);

			fprint_bstr(fp, "Nonce = ", nonce, crypto_aead_NPUBBYTES);
